...
# Importa o sinal `post_migrate`, que é enviado após a execução de migrações do banco de dados.
from django.db.models.signals import post_migrate

# Importa o decorador `receiver` para associar a função a um sinal específico.
from django.dispatch import receiver


''' **IMPORTAÇÕES INTERNAS**  '''
# Importa o modelo `Categoria`, que será usado para preencher dados no banco de dados.
from .models import Categoria




# Define uma função que será executada após as migrações serem aplicadas no banco de dados.
@receiver(post_migrate)
def populate_initial_data(sender, **kwargs):

    # Verifica se já existem dados na tabela `Categoria` para evitar duplicação.
    if not Categoria.objects.exists():

        # Lista contendo os dados dos times e suas respectivas informações.
        times_data = [
            # Times da Série A
        ...
        # Insere os dados dos times na tabela `Categoria`.
